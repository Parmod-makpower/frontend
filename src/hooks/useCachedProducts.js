...
export const useCachedProducts = () => {
  ...
  //
  // OLD:
  // Har product ke liye virtualStockData.find()
  //
  // NEW:
  // Product ID se direct stock lookup
  //
  // Example:
  // 101 -> 50
  // 102 -> 20
  ...
  const virtualStockMap = useMemo(() => {
    return new Map(
      virtualStockData.map((item) => [
        item.product_id,
        item.virtual_stock,
      ])
    );
  ...
  const mumbaiStockMap = useMemo(() => {
    return new Map(
      mumbaiStockData.map((item) => [
        item.product_id,
        item.mumbai_stock,
      ])
    );
  ...
  const mergedProducts = useMemo(() => {
    ...
    return allProducts.map((prod) => {
      // ------------------------------------------------
      // Direct stock lookup using Map
      // ------------------------------------------------

      const virtualStock = virtualStockMap.get(
        prod.product_id
      );

      const mumbaiStock = mumbaiStockMap.get(
        prod.product_id
      );
      ...
      return {
        ...
        // Virtual stock
        //
        // Agar API mein product mila:
        //     API ka latest stock use hoga
        //
        // Agar API mein product nahi mila:
        //     Existing product stock use hoga
        //
        virtual_stock:
